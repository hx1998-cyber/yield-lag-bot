...
def run_paper_signal_replay(
    *,
    ...
    trades.to_csv(out_path, index=False)
    report_path.write_text(_report_markdown(trades, summary_path=summary_path), encoding="utf-8")
    return trades


def _candidate_rows(summary: pd.DataFrame) -> pd.DataFrame:
    required = {
        "cme_symbol",
        "crypto_symbol",
        "best_horizon",
    ...
    return summary[
        summary["cme_symbol"].isin(ALLOWED_CME_SYMBOLS)
    ...
    *,
    ...
    for _, row in replay.iterrows():
        ...
        if abs(cme_return_bps) < min_cme_return_bps:
            continue
        if last_trade_time is not None and signal_time < last_trade_time + cooldown:
            continue
...
def _report_markdown(
    ...
    *,
    ...
    win_rate = float((trades["net_pnl"] > 0).mean()) if total_trades else 0.0
